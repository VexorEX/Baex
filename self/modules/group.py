...
from telethon.tl.functions.messages import PinMessageRequest, UnpinMessageRequest, DeleteMessagesRequest, GetMessagesRequest
...
async def register_group_handlers(client, session_name, owner_id):
    ...
    async def handle_pin_message(event):
        try:
            ...
            if event.is_reply:
                reply_msg = await event.get_reply_message()
                await client(PinMessageRequest(event.chat_id, reply_msg.id, silent=True))
