...
async def register_manage_handlers(client, session_name, owner_id):
    ...
    def get_message(key, **kwargs):
        try:
            ...
            return (
                messages.get("en", {}).get("manage", {}).get(key, "").format(**kwargs)
            )
    ...
    async def forward_to_channel(chat_id, message_id, caption=None):
        ...
        try:
            if (
                not settings["manage_settings"]["channel_id"]
                or not settings["manage_settings"]["channel_username"]
            ):
                return False
            ...
            if messages:
                ...
                if msg.text:
                    msg.text = re.sub(
                        r"@[\w.]+",
                        f"@{settings['manage_settings']['channel_username']}",
                        msg.text,
                    )
                await client(
                ...
                if caption:
                    await client.send_message(
                        settings["manage_settings"]["channel_id"], caption
                    )
                return True
        except Exception as e:
    ...
    async def delete_user_messages(chat_id, user_id, count=10):
        ...
        try:
            ...
            async for message in client.iter_messages(
                chat_id, from_user=user_id, limit=count
            ):
    ...
    async def handle_expiration(event):
        try:
            if event.sender_id != owner_id:
                ...
                return
            expiration = settings["manage_settings"].get(
                "expire_time", "No expiration set"
            )
    ...
    async def handle_create_channel(event):
        try:
            ...
            if channel_id:
                await send_message(
                    event, get_message("channel_created", title=title, id=channel_id)
                )
            else:
            ...
            if event.is_reply:
                ...
                if success:
                    await send_message(
                        event,
                        get_message(
                            "forwarded_to_channel",
                            channel=settings["manage_settings"]["channel_username"],
                        ),
                    )
                else:
                    await send_message(event, get_message("error_occurred"))
        ...
        try:
            ...
            if event.is_reply:
                ...
                deleted_count = await delete_user_messages(
                    event.chat_id, reply_msg.sender_id, count
                )
                if deleted_count:
                    await send_message(
                        event, get_message("messages_deleted", count=deleted_count)
                    )
                else:
        ...
        try:
            ...
            status = {
                ...
                "save": len(
                    [
                        k
                        for k, v in settings["manage_settings"]["save_enabled"].items()
                        if v
                    ]
                ),
            ...
            await send_message(
                event, get_message("self_status", status=status_text), parse_mode="html"
            )
        except Exception as e:
        ...
        try:
            if event.is_reply:
                ...
                if info:
                    ...
                    await send_message(
                        event,
                        get_message("user_info", info=info_text),
                        parse_mode="html",
                    )
                else:
                    await send_message(event, get_message("error_occurred"))
    ...
    async def handle_user_id(event):
        try:
            if event.is_reply:
                ...
                user_id = user.id
            await send_message(
                event, get_message("user_id", id=user_id), parse_mode="html"
            )
        except Exception as e:
    ...
    async def handle_chat_id(event):
        try:
            await send_message(
                event, get_message("chat_id", id=event.chat_id), parse_mode="html"
            )
        except Exception as e:
    ...
    async def handle_channel_info(event):
        try:
            ...
            await send_message(
                event, get_message("channel_info", info=info), parse_mode="html"
            )
        except Exception as e:
    ...
    async def handle_add_contact(event):
        try:
            if event.is_reply:
                ...
                await client(
                    AddContactRequest(
                        user.id, user.first_name, user.phone or "", user.first_name
                    )
                )
                await send_message(
                    event,
                    get_message("contact_added", name=user.first_name),
                    parse_mode="html",
                )
            else:
                await send_message(event, get_message("reply_to_user"))
    ...
    async def handle_panel(event):
        try:
            ...
            keyboard = [
                [
                    Button.inline(
                        "تنظیمات پروفایل", data=f"panel_profile_{chat_id}".encode()
                    )
                ],
                [
                    Button.inline(
                        "تنظیمات عمومی", data=f"panel_settings_{chat_id}".encode()
                    )
                ],
                [Button.inline("لیست‌ها", data=f"panel_lists_{chat_id}".encode())],
            ]
            await send_message(
                event,
                get_message("panel_text"),
                parse_mode="html",
        ...
        try:
            if event.is_reply:
                ...
                keyboard = [
                    [
                        Button.inline(
                            "تنظیم واکنش", data=f"manage_reaction_{user.id}".encode()
                        )
                    ],
                    [
                        Button.inline(
                            "افزودن به سفید", data=f"manage_white_{user.id}".encode()
                        )
                    ],
                    [
                        Button.inline(
                            "افزودن به سیاه", data=f"manage_black_{user.id}".encode()
                        )
                    ],
                ]
                await send_message(
    ...
    async def handle_list_all(event):
        try:
            ...
            await send_message(
                event, get_message("list_all_text", lists=lists_text), parse_mode="html"
            )
        except Exception as e:
    ...
    async def handle_save_message(event):
        try:
            if event.is_reply:
                ...
                await send_message(
                    event,
                    get_message("message_saved", id=reply_msg.id),
                    parse_mode="html",
                )
            else:
                await send_message(event, get_message("reply_to_message"))
    ...
    async def handle_message_to_user(event):
        try:
            ...
            await send_message(
                event,
                get_message("message_sent_to_user", id=user_id),
                parse_mode="html",
            )
        except Exception as e:
    ...
    async def handle_bot_mode_toggle(event):
        try:
            ...
            settings["manage_settings"]["bot_token"] = (
                status if status == "روشن" or status == "on" else None
            )
            await update_settings(db, settings)
            emoji = "✅" if settings["manage_settings"]["bot_token"] else "❌"
            await send_message(
                event,
                get_message("bot_mode_toggle", status=status, emoji=emoji),
                parse_mode="html",
            )
        except Exception as e:
    ...
    async def handle_add_white_list(event):
        try:
            ...
            if user_id not in settings["manage_settings"]["white_list"]:
                ...
                await send_message(
                    event,
                    get_message("white_list_added", id=user_id),
                    parse_mode="html",
                )
            else:
                await send_message(
                    event, get_message("already_in_white_list"), parse_mode="html"
                )
        except Exception as e:
    ...
    async def handle_delete_white_list(event):
        try:
            ...
            if user_id in settings["manage_settings"]["white_list"]:
                ...
                await send_message(
                    event,
                    get_message("white_list_deleted", id=user_id),
                    parse_mode="html",
                )
            else:
                await send_message(
                    event, get_message("not_in_white_list"), parse_mode="html"
                )
        except Exception as e:
    ...
    async def handle_list_white_list(event):
        try:
            ...
            if white_list:
                ...
                await send_message(
                    event, get_message("white_list", list=list_text), parse_mode="html"
                )
            else:
                await send_message(
                    event, get_message("empty_white_list"), parse_mode="html"
                )
        except Exception as e:
    ...
    async def handle_clear_white_list(event):
        try:
            ...
            await update_settings(db, settings)
            await send_message(
                event, get_message("white_list_cleared"), parse_mode="html"
            )
        except Exception as e:
    ...
    async def handle_add_black_list(event):
        try:
            ...
            if user_id not in settings["manage_settings"]["black_list"]:
                ...
                await send_message(
                    event,
                    get_message("black_list_added", id=user_id),
                    parse_mode="html",
                )
            else:
                await send_message(
                    event, get_message("already_in_black_list"), parse_mode="html"
                )
        except Exception as e:
    ...
    async def handle_delete_black_list(event):
        try:
            ...
            if user_id in settings["manage_settings"]["black_list"]:
                ...
                await send_message(
                    event,
                    get_message("black_list_deleted", id=user_id),
                    parse_mode="html",
                )
            else:
                await send_message(
                    event, get_message("not_in_black_list"), parse_mode="html"
                )
        except Exception as e:
    ...
    async def handle_list_black_list(event):
        try:
            ...
            if black_list:
                ...
                await send_message(
                    event, get_message("black_list", list=list_text), parse_mode="html"
                )
            else:
                await send_message(
                    event, get_message("empty_black_list"), parse_mode="html"
                )
        except Exception as e:
    ...
    async def handle_clear_black_list(event):
        try:
            ...
            await update_settings(db, settings)
            await send_message(
                event, get_message("black_list_cleared"), parse_mode="html"
            )
        except Exception as e:
        ...
        try:
            ...
            if event.is_reply:
                ...
                await send_message(
                    event,
                    get_message(
                        "user_reaction_set", user=user.first_name, reaction=reaction
                    ),
                    parse_mode="html",
                )
            else:
        ...
        try:
            ...
            if event.is_reply:
                ...
                if user.id in settings["manage_settings"]["user_reactions"]:
                    ...
                    await send_message(
                        event,
                        get_message("user_reaction_removed", user=user.first_name),
                        parse_mode="html",
                    )
                else:
                    await send_message(
                        event, get_message("no_reaction_set"), parse_mode="html"
                    )
            else:
                await send_message(event, get_message("reply_to_user"))
        except Exception as e:
            ...
            if event.is_reply:
                ...
                if user.id in settings["manage_settings"]["user_reactions_global"]:
                    ...
                    await send_message(
                        event,
                        get_message(
                            "global_user_reaction_removed", user=user.first_name
                        ),
                        parse_mode="html",
                    )
                else:
                    await send_message(
                        event, get_message("no_global_reaction_set"), parse_mode="html"
                    )
            else:
                await send_message(event, get_message("reply_to_user"))
        except Exception as e:
    ...
    async def handle_set_command_prefix(event):
        try:
            ...
            await send_message(
                event,
                get_message("command_prefix_set", prefix=prefix),
                parse_mode="html",
            )
        except Exception as e:
    ...
    async def handle_remove_command_prefix(event):
        try:
            ...
            await update_settings(db, settings)
            await send_message(
                event, get_message("command_prefix_removed"), parse_mode="html"
            )
        except Exception as e:
    ...
    async def handle_toggle_comment(event):
        try:
            ...
            await send_message(
                event,
                get_message("comment_toggle", status=status, emoji=emoji),
                parse_mode="html",
            )
        except Exception as e:
    ...
    async def handle_add_comment(event):
        try:
            ...
            await update_settings(db, settings)
            await send_message(
                event, get_message("comment_added", comment=comment), parse_mode="html"
            )
        except Exception as e:
    ...
    async def handle_delete_comment(event):
        try:
            ...
            if comment in settings["manage_settings"]["comment_list"]:
                ...
                await send_message(
                    event,
                    get_message("comment_deleted", comment=comment),
                    parse_mode="html",
                )
            else:
                await send_message(
                    event, get_message("comment_not_found"), parse_mode="html"
                )
        except Exception as e:
    ...
    async def handle_clear_comments(event):
        try:
            ...
            await update_settings(db, settings)
            await send_message(
                event, get_message("comments_cleared"), parse_mode="html"
            )
        except Exception as e:
    ...
    async def handle_list_comments(event):
        try:
            ...
            if comments:
                ...
                await send_message(
                    event,
                    get_message("comments_list", comments=comments_list),
                    parse_mode="html",
                )
            else:
    ...
    async def handle_get_request_list(event):
        try:
            if event.sender_id != owner_id:
                await send_message(event, get_message("unauthorized"))
                return
            requests = await client.get_participants(
                event.chat_id, filter=ChannelParticipantsRecent
            )
            await send_message(
                event,
                get_message("request_list", count=len(requests)),
                parse_mode="html",
            )
        except Exception as e:
    ...
    async def handle_get_join_list(event):
        try:
            ...
            await send_message(
                event, get_message("join_list", count=len(members)), parse_mode="html"
            )
        except Exception as e:
    ...
    async def handle_get_leave_list(event):
        try:
            ...
            await send_message(
                event, get_message("leave_list", count=0), parse_mode="html"
            )
        except Exception as e:
    ...
    async def handle_approve_requests(event):
        try:
            ...
            await send_message(
                event, get_message("requests_approved"), parse_mode="html"
            )
        except Exception as e:
    ...
    async def handle_monitor_user(event):
        try:
            ...
            if user.id not in settings["manage_settings"]["monitored_users"]:
                ...
                await send_message(
                    event,
                    get_message("user_monitored", user=user.first_name),
                    parse_mode="html",
                )
            else:
                await send_message(
                    event, get_message("already_monitored"), parse_mode="html"
                )
        except Exception as e:
    ...
    async def handle_save_reads(event):
        try:
            ...
            await send_message(
                event,
                get_message("save_reads_toggle", status=status, emoji=emoji),
                parse_mode="html",
            )
        except Exception as e:
    ...
    async def handle_save_stories(event):
        try:
            ...
            await send_message(
                event,
                get_message("save_stories_toggle", status=status, emoji=emoji),
                parse_mode="html",
            )
        except Exception as e:
    ...
    async def handle_add_timer(event):
        try:
            ...
            settings["manage_settings"]["timers"][name] = datetime.now(
                pytz.UTC
            ).timestamp()
            await update_settings(db, settings)
            await send_message(
                event, get_message("timer_added", name=name), parse_mode="html"
            )
        except Exception as e:
    ...
    async def handle_delete_timer(event):
        try:
            ...
            if name in settings["manage_settings"]["timers"]:
                ...
                await update_settings(db, settings)
                await send_message(
                    event, get_message("timer_deleted", name=name), parse_mode="html"
                )
            else:
                await send_message(
                    event, get_message("timer_not_found"), parse_mode="html"
                )
        except Exception as e:
    ...
    async def handle_list_timers(event):
        try:
            ...
            if timers:
                timer_list = "\n".join(
                    [
                        f"{name}: {int((datetime.now(pytz.UTC).timestamp() - start_time) / 60)} minutes"
                        for name, start_time in timers.items()
                    ]
                )
                await send_message(
                    event,
                    get_message("timers_list", timers=timer_list),
                    parse_mode="html",
                )
            else:
    ...
    async def handle_add_time(event):
        try:
            ...
            target = datetime.strptime(target_str, "%Y/%m/%d %H:%M").replace(
                tzinfo=pytz.UTC
            )
            settings["manage_settings"]["times"][name] = target.timestamp()
            await update_settings(db, settings)
            await send_message(
                event, get_message("time_added", name=name), parse_mode="html"
            )
        except Exception as e:
    ...
    async def handle_delete_time(event):
        try:
            ...
            if name in settings["manage_settings"]["times"]:
                ...
                await update_settings(db, settings)
                await send_message(
                    event, get_message("time_deleted", name=name), parse_mode="html"
                )
            else:
                await send_message(
                    event, get_message("time_not_found"), parse_mode="html"
                )
        except Exception as e:
    ...
    async def handle_list_times(event):
        try:
            ...
            if times:
                time_list = "\n".join(
                    [
                        f"{name}: {int((target - datetime.now(pytz.UTC).timestamp()) / 86400)} days left"
                        for name, target in times.items()
                    ]
                )
                await send_message(
                    event, get_message("times_list", times=time_list), parse_mode="html"
                )
            else:
    ...
    async def handle_user_update(event):
        try:
            if event.user_id in settings["manage_settings"]["monitored_users"]:
                monitored = settings["manage_settings"]["monitored_users"][
                    event.user_id
                ]
                if (
                    monitored["online"]
                    and event.online_changed
                    and settings["manage_settings"]["monitor_realm"]
                ):
                    status = "online" if event.is_online else "offline"
                    await client.send_message(
                        settings["manage_settings"]["monitor_realm"],
                        f"User {event.user_id} is now {status}",
                    )
                if (
                    monitored["photo"]
                    ...
                    await client.send_message(
                        settings["manage_settings"]["monitor_realm"],
                        f"User {event.user_id} changed profile photo",
                    )
        except Exception as e:
    ...
    async def handle_message_read(event):
        try:
            if (
                settings["manage_settings"]["save_reads_enabled"]
                and settings["manage_settings"]["read_realm"]
            ):
                await client.send_message(
                    settings["manage_settings"]["read_realm"],
                    f"Message read by {event.user_id}",
                )
        except Exception as e:
    ...
    @client.on(
        events.NewMessage
    )  # StoryPosted event is not directly supported, using NewMessage as fallback
    async def handle_story_posted(event):
        try:
            if (
                settings["manage_settings"]["save_stories_enabled"]
                and settings["manage_settings"]["story_realm"]
                and event.message.is_story
            ):
                await client.forward_messages(
                    settings["manage_settings"]["story_realm"],
                    event.chat_id,
                    [event.message.id],
                )
        except Exception as e:
