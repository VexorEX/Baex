...
async def register_manage_handlers(client, session_name, owner_id):
    ...
    def get_message(key, **kwargs):
        try:
            ...
            return messages.get('en', {}).get('manage', {}).get(key, '').format(**kwargs)
    ...
    async def handle_panel(event):
        try:
            ...
            keyboard = [
                [Button.inline("تنظیمات پروفایل", data=f"panel_profile_{chat_id}".encode())],
                [Button.inline("تنظیمات عمومی", data=f"panel_settings_{chat_id}".encode())],
    ...
    async def handle_manage_user(event):
        try:
            if event.is_reply:
                ...
                user = await client.get_entity(reply_msg.sender_id)
                keyboard = ([
                    [Button.inline("تنظیم واکنش", data=f"manage_reaction_{user.id}".encode())],
                    [Button.inline("افزودن به سفید", data=f"manage_white_{user.id}".encode())],
                    [Button.inline("افزودن به سیاه", data=f"manage_black_{user.id}".encode())]
                ])
