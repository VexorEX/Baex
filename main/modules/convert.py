...
async def register_convert_handlers(client, session_name, owner_id):
    ...
    async def save_cover(cover_path):
        ...
        try:
            if has_db_exec:
                await db.execute(
                    "INSERT OR REPLACE INTO convert_settings (key, value) VALUES (?, ?)",
                    ("cover", cover_path),
                )
                if hasattr(db, "commit"):
                    await db.commit()
        ...
        try:
            if has_db_exec:
                cursor = await db.execute(
                    "SELECT value FROM convert_settings WHERE key = ?", ("cover",)
                )
    ...
    # تنظیم کاور
    @client.on(
        events.NewMessage(pattern=get_command_pattern("set_cover", "convert", lang))
    )
    async def handle_set_cover(event):
        try:
            ...
            cover_path = await upload_to_backup_channel(
                client, settings["convert"]["backup_channel"], file_path
            )
    ...
    # افزودن کاور به آهنگ
    @client.on(
        events.NewMessage(pattern=get_command_pattern("add_cover", "convert", lang))
    )
    async def handle_add_cover(event):
        try:
            ...
            await client.send_file(
                event.chat_id, "output.mp3", attributes=reply_msg.audio.attributes
            )
    ...
    # دریافت کاور
    @client.on(
        events.NewMessage(pattern=get_command_pattern("get_cover", "convert", lang))
    )
    ...
    @client.on(
        events.NewMessage(
            pattern=get_command_pattern("remove_background", "convert", lang)
        )
    )
    async def handle_remove_background(event):
        try:
            ...
            if not REMBG_AVAILABLE:
                await send_message(event, get_message("rembg_not_available"))
                return
    ...
    # تغییر نام فایل/آهنگ
    @client.on(
        events.NewMessage(pattern=get_command_pattern("edit_name", "convert", lang))
    )
        ...
        try:
            ...
            if reply_msg.audio and ":" in name:
                ...
                audio.export(
                    "output.mp3",
                    format="mp3",
                    tags={"artist": artist.strip(), "title": title.strip()},
                )
                await client.send_file(
                    event.chat_id, "output.mp3", attributes=reply_msg.audio.attributes
                )
    ...
    # تغییر زمان
    @client.on(
        events.NewMessage(pattern=get_command_pattern("edit_duration", "convert", lang))
    )
    async def handle_edit_duration(event):
        try:
            ...
            if reply_msg.audio or reply_msg.voice:
                ...
                await client.send_file(
                    event.chat_id, "output.mp3", attributes=reply_msg.media.attributes
                )
            else:
            ...
            os.remove(file_path)
            os.remove(
                "output.mp3" if reply_msg.audio or reply_msg.voice else "output.mp4"
            )
        except Exception as e:
    ...
    # تبدیل استیکر به عکس
    @client.on(
        events.NewMessage(pattern=get_command_pattern("to_photo", "convert", lang))
    )
    ...
    @client.on(
        events.NewMessage(pattern=get_command_pattern("to_gif", "convert", lang))
    )
    ...
    # تبدیل عکس به استیکر
    @client.on(
        events.NewMessage(pattern=get_command_pattern("to_sticker", "convert", lang))
    )
    ...
    # تبدیل به فایل
    @client.on(
        events.NewMessage(pattern=get_command_pattern("to_file", "convert", lang))
    )
    ...
    # تبدیل به ویس
    @client.on(
        events.NewMessage(pattern=get_command_pattern("to_voice", "convert", lang))
    )
    ...
    # تبدیل به آهنگ
    @client.on(
        events.NewMessage(pattern=get_command_pattern("to_audio", "convert", lang))
    )
    ...
    # تبدیل تاریخ
    @client.on(
        events.NewMessage(pattern=get_command_pattern("convert_date", "convert", lang))
    )
        ...
        try:
            ...
            try:
                ...
                await send_message(
                    event,
                    get_message(
                        "date_converted", jalali=jalali_date, gregorian=gregorian_date
                    ),
                )
            except ValueError:
    ...
    # استخراج فایل زیپ
    @client.on(
        events.NewMessage(pattern=get_command_pattern("extract_file", "convert", lang))
    )
    async def handle_extract_file(event):
        try:
            ...
            reply_msg = await event.get_reply_message()
            if (
                not reply_msg.document
                or reply_msg.document.mime_type != "application/zip"
            ):
            ...
            with ZipFile(file_path, "r") as zip_ref:
                ...
                if file_name:
                    if file_name in files:
                        ...
                        await send_message(
                            event, get_message("file_extracted", file_name=file_name)
                        )
                    else:
                        await send_message(
                            event, get_message("file_not_found", file_name=file_name)
                        )
                else:
                    for file in files:
                        await client.send_file(event.chat_id, f"extracted/{file}")
                    await send_message(
                        event, get_message("all_files_extracted", count=len(files))
                    )
            os.remove(file_path)
            for file in files:
    ...
    async def handle_to_qr(event):
        try:
            ...
            text = event.pattern_match.group(1) or (
                event.message.is_reply and (await event.get_reply_message()).text
            )
            if not text:
    ...
    # خواندن QR کد
    @client.on(
        events.NewMessage(pattern=get_command_pattern("read_qr", "convert", lang))
    )
    async def handle_read_qr(event):
        try:
            ...
            if not PYZBAR_AVAILABLE:
                await send_message(event, get_message("pyzbar_not_available"))
                return
    ...
    # تبدیل به ویدئو نوت
    @client.on(
        events.NewMessage(pattern=get_command_pattern("to_video_note", "convert", lang))
    )
    async def handle_to_video_note(event):
        try:
            ...
            video = (
                ...
                .resize((360, 360))
                .set_duration(min(60, reply_msg.video.attributes[0].duration))
            )
            video.write("output.mp4")
    ...
    # تبدیل به ویدئو
    @client.on(
        events.NewMessage(pattern=get_command_pattern("to_video", "convert", lang))
    )
