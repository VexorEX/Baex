...
from telethon.tl.functions.channels import UpdateUsernameRequest as UpdateChannelUsername
...
async def register_profile_handlers(client, session_name, owner_id):
    ...
    async def update_dynamic_text(text, timezone='UTC'):
        """جایگزینی متغیرهای پویا مثل time و date"""
    ...
    async def update_profile_loop(db, settings):
        ...
        while True:
            try:
                ...
                if settings['profile_settings']['bio_enabled'] and settings['profile_settings']['bios']:
                ...
                if settings['profile_settings']['title_enabled'] and settings['profile_settings']['title']:
                    ...
                    async for dialog in client.iter_dialogs():
                        if dialog.is_group and dialog.entity.creator and dialog.entity.id == owner_id:
    ...
    async def user_stats(event):
        try:
            ...
            chats = sum(1 async for _ in client.iter_dialogs())
