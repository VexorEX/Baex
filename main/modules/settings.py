...
from models import init_db, load_settings, update_settings
from telethon import events, types
from telethon.tl.functions.account import UpdateStatusRequest
from telethon.tl.functions.channels import JoinChannelRequest
from telethon.tl.functions.messages import ForwardMessagesRequest, ReadHistoryRequest
from telethon.tl.types import SendMessageTypingAction, SendMessageUploadVideoAction
from utils import get_language, is_command_message, load_json, send_message
...
async def setup_settings(client, db_path):
    ...
    try:
        ...
        await init_db(None)
        print("✅ دیتابیس با موفقیت مقداردهی شد")
        ...
        def get_message(key, **kwargs):
            """Getformatted message for the current language."""
            try:
                ...
                # Fallback toEnglish if the language key doesn't exist
                return (
                    messages.get("en", {})
                    .get("settings", {})
                    .get(key, "")
        ...
        def get_pattern(key):
            """Safeget for command pattern with fallback."""
            return commands.get(lang, {}).get("settings", {}).get(key, "")

            # Helperfunction to toggle settings
        ...
        async def toggle_setting(event, setting_key, chat_id, status):
            if setting_key not in settings:
                ...
                await send_message(
                    event,
                    get_message(f"{setting_key}_toggle", status=status, emoji=emoji),
                )
        ...
        # Commandhandlers (all with get_pattern)
        ...
        async def handle_list_languages(event):
            ...
            await send_message(
                event, get_message("list_languages", languages=languages)
            )
        ...
        @client.on(
            events.NewMessage(pattern=get_pattern("strikethrough_global_toggle"))
        )
        ...
        async def handle_new_message(event):
            ...
            # Check if message is a command - if it is, don't process here
            if await is_command_message(event.message.text, lang, commands):
                return
            ...
            if (
                ...
                or isinstance(event.chat, types.Channel)
                and settings["tick_channel_enabled"]
                or (
                    isinstance(event.chat, types.Chat)
                    and settings["tick_group_enabled"]
                )
            ):
            ...
            if (
                ...
                if message.text:
                    ...
                    await client.send_message(
                        settings["translate_mode_realm"], translated
                    )
        ...
        async def handle_outgoing_message(event):
            ...
            if settings["underline_global_enabled"] or settings[
                "underline_enabled"
            ].get(chat_id, False):
            ...
            # Signature
            if settings["signature_global_enabled"] or settings[
                "signature_enabled"
            ].get(chat_id, False):
        ...
        print(f"❌خطاییدرمقداردهی دیتابیس رخداد:{e}")
