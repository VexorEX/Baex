...
async def register_fast_response_handlers(client, session_name, owner_id):
    ...
    async def handle_add_response(event):
        try:
            ...
            await send_message(event, get_message(f'{mode}_response_added', word=word, response=response, mode=mode))
        except Exception as e:
    ...
    async def handle_delete_response(event):
        try:
            ...
            if await get_response(word, mode):
                ...
                await send_message(event, get_message(f'{mode}_response_deleted', word=word))
            else:
    ...
    async def handle_list_responses(event):
        try:
            ...
            if responses:
                response_list = "\n".join([f"{word}: {response} ({mode})" for word, response, mode in responses])
                await send_message(event, get_message('responses_list', list=response_list))
            else:
    ...
    async def handle_get_response(event):
        try:
            ...
            if result:
                ...
                await send_message(event, get_message('response_info', word=word, mode=mode, response=response))
            else:
                await send_message(event, get_message('response_info_not_found', word=word))
        except Exception as e:
    ...
    async def handle_add_sticker_voice(event):
        try:
            ...
            if event.message.sticker or event.message.voice:
                ...
                if sticker:
                    await send_message(event, get_message('sticker_response_added', word=word))
                elif voice:
                    await send_message(event, get_message('voice_response_added', word=word))
        except Exception as e:
        ...
        try:
            ...
            for word, data in responses.items():
                ...
                if data['mode'] == 'search' and word.lower() not in text:
                    continue
                if data['mode'] in ['normal', 'sudo', 'others', 'reply', 'command'] and text != word.lower():
                    continue
