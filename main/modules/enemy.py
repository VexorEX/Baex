...
async def register_enemy_handlers(client, session_name, owner_id):
    ...
    async def add_enemy(user_id, mode, chat_id=None):
        ...
        try:
            ...
            if has_db_exec:
                ...
                if hasattr(db, "commit"):
                    await db.commit()
            settings["enemy"]["enemies"][mode][user_id] = {
                "chat_id": chat_id,
                "last_insult": timestamp,
            }
            await update_settings(db, settings)
    ...
    async def remove_enemy(user_id, mode, chat_id=None):
        ...
        try:
            if has_db_exec:
                await db.execute(
                    "DELETE FROM enemies WHERE user_id = ? AND mode = ? AND (chat_id = ? OR chat_id IS NULL)",
                    (user_id, mode, chat_id),
                )
                if hasattr(db, "commit"):
                    await db.commit()
    ...
    async def add_insult(text, insult_type="enemy"):
        ...
        try:
            if has_db_exec:
                await db.execute(
                    "INSERT OR REPLACE INTO insults (text, type) VALUES (?, ?)",
                    (text, insult_type),
                )
                if hasattr(db, "commit"):
                    await db.commit()
    ...
    async def remove_insult(text, insult_type="enemy"):
        ...
        try:
            if has_db_exec:
                await db.execute(
                    "DELETE FROM insults WHERE text = ? AND type = ?",
                    (text, insult_type),
                )
                if hasattr(db, "commit"):
                    await db.commit()
            if insult_type == "enemy":
                settings["enemy"]["insults"] = [
                    i for i in settings["enemy"]["insults"] if i != text
                ]
            else:
                settings["enemy"]["friend_insults"] = [
                    i for i in settings["enemy"]["friend_insults"] if i != text
                ]
            await update_settings(db, settings)
        except Exception as e:
        ...
        try:
            if has_db_exec:
                cursor = await db.execute(
                    "SELECT user_id, chat_id FROM enemies WHERE mode = ?", (mode,)
                )
            ...
            return [
                ...
                for uid, data in settings["enemy"]["enemies"].get(mode, {}).items()
            ]
        except Exception as e:
    ...
    async def get_insults(insult_type="enemy"):
        ...
        try:
            if has_db_exec:
                cursor = await db.execute(
                    "SELECT text FROM insults WHERE type = ?", (insult_type,)
                )
                rows = await cursor.fetchall()
                await cursor.close()
    ...
    # فعال کردن فحش
    @client.on(
        events.NewMessage(pattern=get_command_pattern("insult_on", "enemy", lang))
    )
    ...
    # غیرفعال کردن فحش
    @client.on(
        events.NewMessage(pattern=get_command_pattern("insult_off", "enemy", lang))
    )
    ...
    @client.on(
        events.NewMessage(
            pattern=get_command_pattern(
                "set_enemy_(all_chats|pv|groups|this_chat)", "enemy", lang
            )
        )
    )
    async def handle_set_enemy(event):
        try:
            ...
            user_id = event.pattern_match.group(2) or (
                event.message.reply_to_msg_id
                and (await event.get_reply_message()).sender_id
            )
            if not user_id:
    ...
    @client.on(
        events.NewMessage(
            pattern=get_command_pattern(
                "delete_enemy_(all_chats|pv|groups|this_chat)", "enemy", lang
            )
        )
    )
    async def handle_delete_enemy(event):
        try:
            ...
            await send_message(
                event, get_message(f"enemy_{mode}_deleted", user_id=user_id)
            )
        except Exception as e:
    ...
    @client.on(
        events.NewMessage(
            pattern=get_command_pattern(
                "clear_enemy_(all_chats|pv|groups|this_chat)", "enemy", lang
            )
        )
    )
    ...
    @client.on(
        events.NewMessage(
            pattern=get_command_pattern(
                "set_friend_(all_chats|pv|groups|this_chat)", "enemy", lang
            )
        )
    )
    async def handle_set_friend(event):
        try:
            ...
            await add_enemy(user_id, mode, chat_id)
            await send_message(
                event, get_message(f"friend_{mode}_set", user_id=user_id)
            )
        except Exception as e:
    ...
    @client.on(
        events.NewMessage(
            pattern=get_command_pattern(
                "delete_friend_(all_chats|pv|groups|this_chat)", "enemy", lang
            )
        )
    )
    async def handle_delete_friend(event):
        try:
            ...
            await remove_enemy(user_id, mode, chat_id)
            await send_message(
                event, get_message(f"friend_{mode}_deleted", user_id=user_id)
            )
        except Exception as e:
    ...
    @client.on(
        events.NewMessage(
            pattern=get_command_pattern(
                "clear_friend_(all_chats|pv|groups|this_chat)", "enemy", lang
            )
        )
    )
    ...
    # افزودن فحش
    @client.on(
        events.NewMessage(pattern=get_command_pattern("add_insult", "enemy", lang))
    )
    ...
    # حذف فحش
    @client.on(
        events.NewMessage(pattern=get_command_pattern("delete_insult", "enemy", lang))
    )
    ...
    # نمایش لیست فحش‌ها
    @client.on(
        events.NewMessage(pattern=get_command_pattern("list_insults", "enemy", lang))
    )
    ...
    # پاکسازی فحش‌ها
    @client.on(
        events.NewMessage(pattern=get_command_pattern("clear_insults", "enemy", lang))
    )
    ...
    @client.on(
        events.NewMessage(
            pattern=get_command_pattern("add_friend_insult", "enemy", lang)
        )
    )
    ...
    @client.on(
        events.NewMessage(
            pattern=get_command_pattern("delete_friend_insult", "enemy", lang)
        )
    )
    ...
    @client.on(
        events.NewMessage(
            pattern=get_command_pattern("list_friend_insults", "enemy", lang)
        )
    )
    async def handle_list_friend_insults(event):
        try:
            ...
            if insults:
                insult_list = "\n".join(insults)
                await send_message(
                    event, get_message("friend_insult_list", list=insult_list)
                )
            else:
    ...
    @client.on(
        events.NewMessage(
            pattern=get_command_pattern("clear_friend_insults", "enemy", lang)
        )
    )
    ...
    # آپلود فحش از فایل
    @client.on(
        events.NewMessage(pattern=get_command_pattern("upload_insult", "enemy", lang))
    )
    async def handle_upload_insult(event):
        try:
            ...
            if event.message.document:
                ...
                await send_message(
                    event, get_message("insults_uploaded", count=len(insults))
                )
                os.remove(file)
            else:
    ...
    @client.on(
        events.NewMessage(
            pattern=get_command_pattern("upload_friend_insult", "enemy", lang)
        )
    )
    async def handle_upload_friend_insult(event):
        try:
            ...
            if event.message.document:
                ...
                await send_message(
                    event, get_message("friend_insults_uploaded", count=len(insults))
                )
                os.remove(file)
            else:
                await send_message(event, get_message("no_file"))
    ...
    # تنظیم زمان تأخیر
    @client.on(
        events.NewMessage(pattern=get_command_pattern("set_delay", "enemy", lang))
    )
    ...
    @client.on(
        events.NewMessage(pattern=get_command_pattern("delay_on", "enemy", lang))
    )
    ...
    @client.on(
        events.NewMessage(pattern=get_command_pattern("delay_off", "enemy", lang))
    )
    ...
    # تنظیم مکث تایپینگ
    @client.on(
        events.NewMessage(pattern=get_command_pattern("set_pause", "enemy", lang))
    )
    async def handle_set_pause(event):
        try:
            ...
            await update_settings(db, settings)
            await send_message(
                event, get_message("pause_set", min=min_pause, max=max_pause)
            )
        except Exception as e:
    ...
    @client.on(
        events.NewMessage(
            pattern=get_command_pattern("delete_enemy_message_on", "enemy", lang)
        )
    )
    ...
    @client.on(
        events.NewMessage(
            pattern=get_command_pattern("delete_enemy_message_off", "enemy", lang)
        )
    )
    ...
    # تنظیم امضا
    @client.on(
        events.NewMessage(pattern=get_command_pattern("set_signature", "enemy", lang))
    )
    ...
    @client.on(
        events.NewMessage(
            pattern=get_command_pattern("delete_signature", "enemy", lang)
        )
    )
    ...
    @client.on(
        events.NewMessage(
            pattern=get_command_pattern("set_friend_signature", "enemy", lang)
        )
    )
    async def handle_set_friend_signature(event):
        try:
            ...
            await update_settings(db, settings)
            await send_message(
                event, get_message("friend_signature_set", signature=signature)
            )
        except Exception as e:
    ...
    @client.on(
        events.NewMessage(
            pattern=get_command_pattern("delete_friend_signature", "enemy", lang)
        )
    )
    ...
    # نمایش لیست‌ها
    @client.on(
        events.NewMessage(pattern=get_command_pattern("list_all", "enemy", lang))
    )
            ...
            for mode in ["all_chats", "pv", "groups", "this_chat"]:
                ...
                if enemies:
                    enemy_lists.append(
                        ...
                        + "\n".join(
                            [
                                f"User {user_id} (Chat: {chat_id or 'N/A'})"
                                for user_id, chat_id in enemies
                            ]
                        )
                    )
                ...
                if friends:
                    enemy_lists.append(
                        ...
                        + "\n".join(
                            [
                                f"User {user_id} (Chat: {chat_id or 'N/A'})"
                                for user_id, chat_id in friends
                            ]
                        )
                    )
            insults = await get_insults("enemy")
    ...
    @client.on(
        events.NewMessage(
            pattern=get_command_pattern("clear_all_enemies", "enemy", lang)
        )
    )
    async def handle_clear_all_enemies(event):
        try:
            if event.sender_id != owner_id:
                await send_message(event, get_message("unauthorized"))
                return
            user_id = event.pattern_match.group(1) or (
                event.message.reply_to_msg_id
                and (await event.get_reply_message()).sender_id
            )
            if not user_id:
            ...
            await send_message(
                event, get_message("all_enemies_cleared", user_id=user_id)
            )
        except Exception as e:
    ...
    # دریافت اطلاعات دشمن
    @client.on(
        events.NewMessage(pattern=get_command_pattern("get_enemy", "enemy", lang))
    )
    async def handle_get_enemy(event):
        try:
            ...
            for mode in ["all_chats", "pv", "groups", "this_chat"]:
                if (
                    user_id in settings["enemy"]["enemies"][mode]
                    or user_id in settings["enemy"]["friends"][mode]
                ):
                    modes.append(mode)
            if modes:
                await send_message(
                    event,
                    get_message("enemy_info", user_id=user_id, modes=", ".join(modes)),
                )
            else:
                await send_message(
                    event, get_message("enemy_not_found", user_id=user_id)
                )
        except Exception as e:
        ...
        try:
            ...
            for mode, enemies in settings["enemy"]["enemies"].items():
                if sender_id in enemies:
                    if (
                        mode == "all_chats"
                        or (mode == "pv" and event.is_private)
                        or (mode == "groups" and event.is_group)
                        or (
                            mode == "this_chat"
                            and enemies[sender_id]["chat_id"] == chat_id
                        )
                    ):
                        if (
                            settings["enemy"]["delay_enabled"]
                            and (current_time - enemies[sender_id]["last_insult"])
                            < settings["enemy"]["delay_time"]
                        ):
                            continue
            ...
            for mode, friends in settings["enemy"]["friends"].items():
                if sender_id in friends:
                    if (
                        ...
                        or (
                            mode == "this_chat"
                            and friends[sender_id]["chat_id"] == chat_id
                        )
                    ):
                        if (
                            settings["enemy"]["delay_enabled"]
                            and (current_time - friends[sender_id]["last_insult"])
                            < settings["enemy"]["delay_time"]
                        ):
                            continue
                        if settings["enemy"]["delete_enemy_messages"]:
