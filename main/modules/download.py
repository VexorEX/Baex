...
async def register_download_handlers(client, session_name, owner_id):
    ...
    async def save_download_history(user_id, query, download_type):
        try:
            if has_db_exec:
                await db.execute(
                    "INSERT INTO download_history (user_id, query, type) VALUES (?, ?, ?)",
    ...
    @client.on(
        events.NewMessage(
            pattern=get_command_pattern("download_song", "download", lang)
        )
    )
    async def handle_download_song(event):
        try:
            ...
            async for msg in client.iter_messages(
                None, search=query, filter=types.InputMessagesFilterMusic, limit=1
            ):
                if msg.audio:
                    file_path = await msg.download_media()
                    await client.send_file(
                        event.chat_id, file_path, attributes=msg.audio.attributes
                    )
                    await send_message(
                        event, get_message("song_downloaded", query=query)
                    )
                    os.remove(file_path)
    ...
    @client.on(
        events.NewMessage(
            pattern=get_command_pattern("download_video", "download", lang)
        )
    )
            ...
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                if os.path.exists(file_path):
                    if os.path.getsize(file_path) > 200 * 1024 * 1024:  # 200 مگابایت
                        ...
                        return
                    await client.send_file(
                        event.chat_id, file_path, caption=info.get("title", "")
                    )
                    await send_message(
                        event,
                        get_message("video_downloaded", title=info.get("title", "")),
                    )
                    os.remove(file_path)
    ...
    @client.on(
        events.NewMessage(
            pattern=get_command_pattern("download_list", "download", lang)
        )
    )
    async def handle_download_list(event):
        try:
            ...
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                ...
                if not videos:
                    await send_message(
                        event, get_message("no_videos_found", query=query)
                    )
                    return
                response = (
                    f"📋 {get_message('video_list', count=len(videos), query=query)}\n"
                )
    ...
    @client.on(
        events.NewMessage(
            pattern=get_command_pattern("download_all_stories", "download", lang)
        )
    )
    async def handle_download_all_stories(event):
        try:
            ...
            for story in stories:
                ...
                count += 1
            await send_message(
                event, get_message("all_stories_downloaded", count=count)
            )
    ...
    @client.on(
        events.NewMessage(
            pattern=get_command_pattern("download_stories", "download", lang)
        )
    )
    async def handle_download_stories(event):
        try:
            ...
            if not stories:
                await send_message(
                    event, get_message("no_stories_found_for_user", username=username)
                )
                return
            count = 0
            for story in stories:
                ...
                count += 1
            await send_message(
                event, get_message("stories_downloaded", username=username, count=count)
            )
    ...
    # انتشار استوری
    @client.on(
        events.NewMessage(pattern=get_command_pattern("new_story", "download", lang))
    )
    ...
    @client.on(
        events.NewMessage(
            pattern=get_command_pattern("download_file", "download", lang)
        )
    )
    async def handle_download_file(event):
        try:
            ...
            await send_message(
                event, get_message("file_downloaded", file_name=file_name)
            )
            os.remove(file_name)
    ...
    # دانلود مخزن GitHub
    @client.on(
        events.NewMessage(pattern=get_command_pattern("download_git", "download", lang))
    )
    async def handle_download_git(event):
        try:
            ...
            await send_message(
                event, get_message("git_downloaded", repo_name=repo_name)
            )
            os.remove(zip_name)
            shutil.rmtree(temp_dir)
